...
| 合成结果为空（无新料） | 不写文件，但**更新 hash**——这版 STATUS 已处理过，没必要天天重爬（precision-first） |
| 爬/合成/投递出错（异常） | 该项目跳过、**不更新 hash**（下次重试），不影响其他项目 |
| onboard 时情报文件不存在 | 跳过，汇报"暂无谛听项目情报" |

> **hash 更新规则一句话**：只要"成功处理完这版 STATUS"（爬+合成跑通，且有料就投递成功 / 没料就不投递）就更新 hash；任何异常（爬不动/合成失败/写文件失败）都不更新，留到下次重试。
