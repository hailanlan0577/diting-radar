...
新建 `src/diting/project_radar.py`（本任务先只写 `detect_changed_projects`；Task 7 再补 `run_project_radar`）：
...
- Produces: `run_project_radar(cfg, client, store, *, now_ts=None, sources=None, feishu_run=subprocess.run, enrich=enrich_bodies) -> list[Report]`。
...
```python
from diting.models import Candidate
from diting.state import StateStore as _SS  # 已在文件顶部 import，这里复用
...
在 `src/diting/project_radar.py` 末尾（`detect_changed_projects` 之后）加：

```python
def run_project_radar(cfg, client, store, *, now_ts=None, sources=None,
                      feishu_run=subprocess.run, enrich=enrich_bodies) -> list[Report]:
...
> 注：`feishu_run` 参数暂保留为接口一致性占位（项目雷达第一版不发飞书，只写 Obsidian——onboard 时才读）。若后续要加飞书短通知，在此用它。
