...
```toml
dependencies = ["httpx>=0.27", "pyyaml>=6.0", "trafilatura>=1.12", "lxml_html_clean>=0.1", "scrapling>=0.4.9"]
```
...
- Produces: `fetch_text(url: str, *, stealthy: bool = False, fetcher=None, timeout_s: int = 30) -> str` — 返回正文纯文本；失败/空一律返回 `""`，绝不抛出。`fetcher` 为可注入的 `callable(url, stealthy, timeout_s) -> html_str`，默认用 scrapling。
...
**Interfaces:**
- Produces: `search_engine(query: str, *, max_results: int = 5, serp_fetcher=None, timeout_s: int = 30) -> list[Candidate]` — 抓 DDG HTML 版结果解析为 `source="websearch"` 的 `Candidate`（无 body）；失败返 `[]`。`serp_fetcher` 为可注入 `callable(query, max_results, timeout_ms) -> html_str`。
