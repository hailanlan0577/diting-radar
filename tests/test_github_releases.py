...
def test_first_sighting_returns_one_candidate_and_writes_snapshot(tmp_path):
    ...
    result = check_repo_release("owner/repo", store, get=mock_get)
...
def test_newer_version_returns_candidate(tmp_path):
    ...
    assert len(result) == 1
    assert "v2.0.0" in result[0].title
    assert store.get_seen_version("owner/repo") == "v2.0.0"
...
def test_no_release_body_uses_name(tmp_path):
    ...
    assert len(result) == 1
    assert result[0].summary == "My Release"
